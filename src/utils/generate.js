...
export async function generateSwap(originalZip, plates, loopRepeats, outputName, coverPlate, opts = {}) {
    const {
        disableMechModeCheck = false,
        disableMechModeCheckAll = false,
        disableDynamicFlow = false,
        express = false,
        onProgress,
        controller
    } = opts;
    ...
    // ── 2. Read gcode as text (with cache per plate id), mask slicer params ──
    const gcodeCache = {};
    const gcodeEntries = [];
    const maskCache = {};
    ...
    for (let i = 0; i < baseQueue.length; i++) {
        ...
        if (!gcodeCache[plate.id]) {
            const raw = await readGcodeText(plate.zip || originalZip, plate);
            const {masked, placeholders} = maskParamGcode(raw);
            gcodeCache[plate.id] = masked;
            maskCache[plate.id] = placeholders;
        }
    ...
    // ── 10. Remap object IDs in gcode to match queue positions ──────────────
    for (let i = 0; i < loopedEntries.length; i++) {
        const plate = baseQueue[i % baseQueue.length];
        const pos = i + 1;
        for (const obj of plate.objects || []) {
            const origId = obj.identifyId;
            const newId = `${pos}${origId}`;
            loopedEntries[i] = loopedEntries[i]
                .replaceAll(`; OBJECT_ID: ${origId}`, `; OBJECT_ID: ${newId}`)
                .replaceAll(`; start printing object, unique label id: ${origId}`, `; start printing object, unique label id: ${newId}`)
                .replaceAll(`; stop printing object, unique label id: ${origId}`, `; stop printing object, unique label id: ${newId}`);
        }
    }

    // ── 11. Unmask slicer param lines & create gcode blob ──────────────────
    for (let i = 0; i < loopedEntries.length; i++) {
        const plate = baseQueue[i % baseQueue.length];
        loopedEntries[i] = unmaskParamGcode(loopedEntries[i], maskCache[plate.id]);
    }
    ...
    // Build queue objects with position-prefixed IDs
    const fullQueue = [];
    for (let i = 0; i < loop; i++) fullQueue.push(...baseQueue);
    const queueObjects = [];
    for (let i = 0; i < fullQueue.length; i++) {
        for (const obj of fullQueue[i].objects || []) {
            queueObjects.push({
                identifyId: `${i + 1}${obj.identifyId}`,
                name: obj.name,
                skipped: obj.skipped,
            });
        }
    ...
    if (plates.length === 1) {
        newZip.file('Metadata/slice_info.config', await patchSinglePlateSliceInfo(originalZip, plates[0], loopRepeats, queueObjects));
    } else {
        newZip.file('Metadata/slice_info.config', await buildMultiPlateSliceInfo(plates, loopRepeats, originalZip, queueObjects));
    }
...
// ── Param gcode masking ──────────────────────────────────────────────────────
// Slicer embeds actual gcode inside comment-parameters like:
//   ; machine_end_gcode = ;===START: FIN===\nG28 XY\n;===END: FIN===
// These must be ignored during marker search/replace.

function maskParamGcode(gcode) {
    const placeholders = [];
    const masked = gcode.replace(/^; machine_(start|end)_gcode = .*/gm, (match) => {
        placeholders.push(match);
        return `; __MASKED_PARAM_${placeholders.length - 1}__`;
    });
    return {masked, placeholders};
}

function unmaskParamGcode(gcode, placeholders) {
    return gcode.replace(/; __MASKED_PARAM_(\d+)__/g, (_, idx) => placeholders[Number(idx)]);
}
...
async function generateExpressInPlace(originalZip, plates, {onProgress: report, check}) {
    ...
    for (let i = 0; i < plates.length; i++) {
        ...
        // Mask slicer param lines that contain embedded gcode
        const {masked, placeholders} = maskParamGcode(gcode);
        gcode = masked;
        ...
        // Unmask slicer param lines
        gcode = unmaskParamGcode(gcode, placeholders);
 ...
 */
async function patchSinglePlateSliceInfo(originalZip, plate, loopRepeats, queueObjects = []) {
    ...
    // Replace objects with queue-expanded ones
    for (const obj of targetPlate.querySelectorAll('object')) obj.remove();
    for (const obj of queueObjects) {
        const el = doc.createElement('object');
        el.setAttribute('identify_id', obj.identifyId);
        el.setAttribute('name', obj.name);
        el.setAttribute('skipped', obj.skipped);
        targetPlate.appendChild(el);
        targetPlate.appendChild(doc.createTextNode('\n    '));
    }
 ...
 */
async function buildMultiPlateSliceInfo(plates, loopRepeats, originalZip, queueObjects = []) {
    const loop = Math.max(1, loopRepeats);
    ...
    const objectsXml = queueObjects.map((obj) => {
            return `    <object identify_id="${obj.identifyId}" name="${obj.name}" skipped="${obj.skipped}"/>`;
        }
    ).join('\n');
